...
<div style="width: 50%; margin: 20px auto;">
    ...
    <form method="post" action="${pageContext.request.contextPath}/products?action=update">
        ...
        <div class="form-group">
            ...
            <input type="text" id="price" name="price" value="<fmt:formatNumber value='${product.price}' type='number' groupingUsed='false'/>">
...
<script>
    ...
    // Định dạng giá trị ban đầu khi tải trang
    function formatInitialValue() {
        let value = priceInput.value.replace(/,/g, '');
        if (!isNaN(value) && value.length > 0) {
            priceInput.value = parseFloat(value).toLocaleString('en-US');
        }
    }
    ...
    priceInput.addEventListener('input', function (e) {
        ...
        if (!isNaN(value) && value.length > 0) {
            ...
            e.target.value = ''; // Xóa nếu không phải là số
        }
    });

    // Gọi hàm định dạng ban đầu khi trang tải xong
    window.onload = formatInitialValue;
</script>
